...
local function part_title_from_single_line_text(s)
  ...
  -- "Part 3: Title" / "Book IV. Title"
  local title = t:match("^[Pp][Aa][Rr][Tt]%s+[%dIVXLCDMivxlcdm]+%s*[:%.%-]%s*(.+)$")
  if not title or title == "" then
    title = t:match("^[Bb][Oo][Oo][Kk]%s+[%dIVXLCDMivxlcdm]+%s*[:%.%-]%s*(.+)$")
  end
  if title and title ~= "" then
    local tt = squash(title)
    if #tt > 0 and #tt <= 120 and has_letters(tt) then
      return tt
    end
  end
  return nil
...
local function looks_like_author_name(s)
  ...
  local tl = lower(t)
  if tl:match("^by%s") or tl:match("%sby%s") then return false end
...
local function looks_like_marketing_start_text(s)
  ...
  while i <= #blocks do
    ...
    if b.t == "Header" then
      ...
      -- Convert single-line "Part N: Title" headers into real parts.
      if lvl == 1 then
    ...
    if looks_like_part_number_then_title(blocks, i) then
      ensure_mainmatter()
...
-- PART inference (title + horizontal rule)
    ...
    if looks_like_part_before_preface(blocks, i) then
      local title = strip_double_braces(stringify(b))
      ensure_mainmatter()
      table.insert(out, make_header(1, title, b.attr))
      current_chapter = nil
    ...
    if (b.t == "Para" or b.t == "Plain") then
      ...
      if st then
        ...
        if st2 then
          local author = strip_double_braces(squash(st2))
          if looks_like_author_name(author) and not lower(t):match("^edited%s+by") then
